 ...
 * - Test with lot's of files that this works
 */
...
SoundSourceFFmpeg::~SoundSourceFFmpeg() {
    ...
    if( m_pSwrCtx != NULL ) {
        qDebug() << "~SoundSourceFFmpeg: Freeing Resample context";
#ifndef __FFMPEGOLDAPI__
            avresample_close(/**/m_pSwrCtx);
#else
            audio_resample_close(m_pSwrCtx);
            //av_resample_close(m_pSwrCtx);
#endif
    }
    /*av_free(pFrame);
      // Close the codec
...
unsigned int SoundSourceFFmpeg::reSample(AVFrame *inframe) {
    ...
    if ((m_pCodecCtx->sample_fmt != AV_SAMPLE_FMT_S16 || m_pCodecCtx->sample_rate != 44100 || m_pCodecCtx->channel_layout != AV_CH_LAYOUT_STEREO) && m_pSwrCtx == NULL ) {
        if( m_pSwrCtx != NULL ) {
...
// __FFMPEGOLDAPI__ Is what is used in FFMPEG < 0.10 and libav < 0.8.3. NO libresample available..
        ...
        if ( avresample_open(m_pSwrCtx ) < 0) {
            ...
            qDebug() << "To " << m_pCodecCtx->sample_rate << " HZ format:" << av_get_sample_fmt_name(AV_SAMPLE_FMT_S16) << " with 2 channels";
            return -1;
        }
#endif
        ...
        qDebug() << "To " << m_pCodecCtx->sample_rate << " HZ format:" <<  av_get_sample_fmt_name(AV_SAMPLE_FMT_S16) << "with 2 (layout:" << m_pCodecCtx->channel_layout << ") channels (BPS " << av_get_bytes_per_sample(AV_SAMPLE_FMT_S16) << ")";
    }
...
int l_iLen = 0;
      ...
      m_pOutSize = av_samples_get_buffer_size(NULL, 2,l_iLen,AV_SAMPLE_FMT_S16, 1);
        ...
        if( av_read_frame(m_pFormatCtx, &l_SPacket) >= 0) {
            if(l_SPacket.stream_index==m_iAudioStream) {
                ...
                if(frameFinished) {
                    ...
                    if (m_pSwrCtx) {
                        readBuffer.write((const char *)(m_pOut), m_pOutSize);
                        readBytes = m_pOutSize;
                        av_freep(&m_pOut);
                        m_pOut = NULL;
                        m_pOutSize = 0;
                    } else {

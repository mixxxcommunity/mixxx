...
void RecordingManager::startRecording(bool generateFileName) {
    ...
    QString encodingType = m_settings.value("Recording/Encoding").toString();
    ...
    if(generateFileName) {
        ...
        m_settings.setValue("Recording/Path", m_recordingLocation);
        m_settings.setValue("Recording/CuePath", m_recording_base_file + ".cue");
    } else {
...
void RecordingManager::setRecordingDir() {
    QDir recordDir(m_settings.value("Recording/Directory").toString());

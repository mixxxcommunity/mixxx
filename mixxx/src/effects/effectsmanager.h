...
class EffectsManager : public QObject {
  ...
  public:
    ...
    // To access the only existing instance of EffectsManager
    // from any part of the Mixxx
    static EffectsManager* getEffectsManager ();
  ...
  private slots:
    ...
    void loadNewlyAddedChain(const unsigned int iChainSlotNumber);
    void deleteAndLoadNextChain(const unsigned int iChainSlotNumber);
  ...
  private:
    ...
    QString newChainId() {
        return QString("org.mixxx.effectchain.%1").arg(++iNewChainNumber);
    }

    QString newChainName() const {
        return QString("new chain %1").arg(iNewChainNumber);
    }
    ...
    static EffectsManager *m_thisPointer;

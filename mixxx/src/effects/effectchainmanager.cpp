...
EffectChainPointer EffectChainManager::deleteAndGetNextEffectChain(EffectChainPointer pEffectChain) {
    EffectChainPointer pNextEffectChain;
    if (m_effectChains.size () != 1)
        pNextEffectChain = getNextEffectChain (pEffectChain);
    m_effectChains.removeAll (pEffectChain);
    return pNextEffectChain;
}

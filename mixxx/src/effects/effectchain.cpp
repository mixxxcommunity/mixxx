...
void EffectChain::deleteEffect(EffectPointer pEffect) {
    qDebug() << debugString() << "deleteEffect";
    QMutexLocker locker(&m_mutex);
    m_effects.removeAll(pEffect);
    locker.unlock();
    emit(updated());
}

void EffectChain::moveEffect(EffectPointer pEffect, int deltaPos) {
    qDebug() << debugString() << "moveEffect";
    QMutexLocker locker(&m_mutex);

    int currentPos = m_effects.indexOf(pEffect);
    int newPos = currentPos + deltaPos;
    if (newPos < 0)
        newPos = 0;
    else if (newPos >= m_effects.size())
        newPos = m_effects.size() - 1;

    m_effects.move(currentPos, newPos);

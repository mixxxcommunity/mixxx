...
EffectChainSlot::EffectChainSlot(QObject* pParent, unsigned int iChainNumber)
    ...
    m_pControlAddChain = new ControlObject(ConfigKey(m_group, "add_chain"));
    connect(m_pControlAddChain, SIGNAL(valueChanged(double)),
            this, SLOT(slotControlAddChain(double)));

    m_pControlDeleteChain = new ControlObject(ConfigKey(m_group, "delete_chain"));
    connect(m_pControlDeleteChain, SIGNAL(valueChanged(double)),
            this, SLOT(slotControlDeleteChain(double)));
...
EffectChainSlot::~EffectChainSlot() {
    ...
    delete m_pControlAddChain;
    delete m_pControlDeleteChain;
...
void EffectChainSlot::loadEffectChain(EffectChainPointer pEffectChain) {
    ...
    if (pEffectChain) {
        ...
        connect (m_pEffectChain.data(), SIGNAL(nameChanged(QString)),
                this, SIGNAL (chainRenamed(QString)));
    ...
    locker.unlock();
    emit(effectChainLoaded(pEffectChain, m_iChainNumber));
    emit(updated());
}
...
void EffectChainSlot::addEffectSlot() {
    ...
    connect(pEffectSlot, SIGNAL(deleteEffect(EffectPointer, unsigned int)),
            this, SLOT(slotDeleteEffect(EffectPointer, unsigned int)));
    connect(pEffectSlot, SIGNAL(moveEffect(EffectPointer, unsigned int, int)),
            this, SLOT(slotMoveEffect(EffectPointer, unsigned int, int)));
...
void EffectChainSlot::slotDeleteEffect(EffectPointer pEffect, unsigned int slotNumber) {
    qDebug() << debugString() << "slotDeleteEffect" << slotNumber;
    QMutexLocker locker(&m_mutex);
    m_pEffectChain->deleteEffect (pEffect);
}

void EffectChainSlot::slotMoveEffect(EffectPointer pEffect, unsigned int slotNumber, int deltaPos) {
    qDebug() << debugString() << "slotMoveEffect";
    QMutexLocker locker(&m_mutex);
    m_pEffectChain->moveEffect (pEffect, deltaPos);
}
...
void EffectChainSlot::slotControlAddChain(double v) {
    qDebug() << debugString() << "slotControlAddChain" << v;
    //QMutexLocker locker(&m_mutex);
    // const int read is not worth locking for
    if (v > 0)
        emit(addChain(m_iChainNumber));
}

void EffectChainSlot::slotControlDeleteChain(double v) {
    qDebug() << debugString() << "slotControlDeleteChain" << v;
    //QMutexLocker locker(&m_mutex);
    // const int read is not worth locking for
    if (v > 0)
        emit(deleteChain(m_iChainNumber));
}

void EffectChainSlot::slotAddEffectToChain(QString effectName) {
    
    qDebug() << debugString() << "Request to add effect" << effectName;
    
    if (m_pEffectChain.isNull ()) 
    {
        qDebug() << debugString() << "Create a chain to load new effect.";
        return;
    }
    
    EffectPointer pEffect = m_pEffectsManager->instantiateEffect (effectName);
    if (pEffect.isNull ())
    {
        qDebug() << debugString() << "Cannot instantiate Effect.";
        return;
    }

    m_pEffectChain->addEffect (pEffect);

}

void EffectChainSlot::slotRenameChain (QString newName) {
    qDebug() << debugString() << "Request to rename chain to " << newName;

    if (m_pEffectChain.isNull ())
    {
        qDebug() << debugString() << "Load a chain to rename";
        return;
    }

    m_pEffectChain->setName (newName);
}

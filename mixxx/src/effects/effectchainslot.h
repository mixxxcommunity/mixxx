...
class EffectChainSlot : public QObject {
  ...
  signals:
    ...
    // Indicates that the given EffectChain was loaded into 
    //ffectChainSlot chainNumber
    void effectChainLoaded(EffectChainPointer pEffectChain, unsigned int chainNumber);
    ...
    // Signal that whoever is in charge of this EffectChainSlot should add and load
    // new EffectChain into it.
    void addChain(const unsigned int iChainSlotNumber);

    // Signal that whoever is in charge of this EffectChainSlot should add and load
    // new EffectChain into it.
    void deleteChain(const unsigned int iChainSlotNumber);

    // Signal that indicates that the EffectChainSlot has been updated.
    void updated();

    // Signal that name of loaded chain has changed
    void chainRenamed (QString);

  public slots:
    // To add a effect in currently loaded chain
    void slotAddEffectToChain(QString effectName);

    // To rename loaded chain
    void slotRenameChain (QString newName);

  private slots:
    void slotChainUpdated();
    void slotEffectLoaded(EffectPointer pEffect, unsigned int slotNumber);
    void slotDeleteEffect(EffectPointer pEffect, unsigned int slotNumber); 
    void slotMoveEffect(EffectPointer pEffect, unsigned int slotNumber, int deltaPos); 
    ...
    void slotControlAddChain(double v);
    void slotControlDeleteChain(double v);
  ...
  private:
    ...
    ControlObject* m_pControlAddChain;
    ControlObject* m_pControlDeleteChain;
